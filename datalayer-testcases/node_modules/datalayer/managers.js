...
// let resultSet = await connection.execute("select * from ac_item order by code desc");
let resultSet = await connection.execute("select code,name, hsn_code,sgst,cgst,igst from ac_item left join ac_item_tax on item_code=code order by name");
let items = [];
let item;
let row;
let i=0;
while(i < resultSet.rows.length) {
row = resultSet.rows[i];
item = new entities.Item(parseInt(row[0]), row[1]?.trim(), parseInt(row[2]), parseInt(row[3]), parseInt(row[4]), parseInt(row[5]));
items.push(item);
i++;
}
await connection.close();
...
let item = new entities.Item(parseInt(row[0]), row[1]?.trim(), parseInt(row[2]), parseInt(tax_row[0]), parseInt(tax_row[1]), parseInt(tax_row[2]));
...
resultSet = await connection.execute(`select item_code from ac_item_tax where item_code=${code}`);
if(resultSet.rows.length > 0) {
await connection.execute(`delete from ac_item_tax where item_code=${code}`);
await connection.commit();
}

resultSet = await connection.execute(`select item_code from ac_item_uom where item_code=${code}`);
if(resultSet.rows.length > 0) {
...
let resultSet = await connection.execute("select * from ac_customer order by code desc");
let customers = [];
let customer;
let row;
let i=0;
...
await connection.execute(`delete from ac_customer where code=${code}`);
await connection.commit();
